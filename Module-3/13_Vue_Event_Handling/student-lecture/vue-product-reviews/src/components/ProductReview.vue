...
Features to add:
  TODO 01: Show the average rating and the frequency of each rating in the "well-display" above the reviews.
    Average Rating:
       Make a computed property for average rating
  ...
  TODO 02: Allow the user to click on a Well to display only those reviews with a particular rating
    Make a filter property that says what-star rating reviews we want to see. filter=0 means see all reviews.
    Make a computed filteredReviews property which applies the filter value
    Listen for the click even on the wells, and change the filter
  ...
  TODO 04: Provide a form to allow the user to add a new review
    Create a newReview variable to store the values for the new review fields that the user enters
    Create the AddReview form,
  ...
  TODO 05: Show and hide the AddReview form as appropriate
   Hide the form by default
      Conditionally show the form based on the value of the data variable (showForm)
...
<template>
  <div class="main">
    ...
    <a href="#" v-on:click.prevent="showForm = true" v-show="!showForm">Add Review</a>
    <!-- TODO 04B: Create the form that allows the user to add a new review -->
    <form v-on:submit.prevent="addNewReview" v-show="showForm">
        <!-- form has: reviewer, title, rating, review, favorite. Object also has Id and favorite -->      
      <div>
      ...
      <div>
        ...
        <input id= "rating" type="number" min="1" max="5" v-model.number="newReview.rating"/>
      </div>
      <div>
...
export default {
  ...
  computed: {
    averageRating(){
      //Calculate the average rating for all the reviews (add ratings of all reviews then divide by count)
